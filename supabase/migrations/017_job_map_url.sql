...
alter table public.jobs
  drop constraint if exists jobs_map_url_https_check;

alter table public.jobs
  add constraint jobs_map_url_https_check
...
declare e uuid;c text;j jobs;result uuid;lat double precision;lng double precision;precision_value text;map_link text;
begin
 ...
 map_link=nullif(trim(p->>'map_url'),'');
 if map_link is not null and map_link !~* '^https://' then raise exception 'INVALID_MAP_URL';end if;
 ...
 latitude=lat,longitude=lng,location_precision=precision_value,map_url=map_link,
