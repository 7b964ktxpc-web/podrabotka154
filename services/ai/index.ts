...
const STREET_PREFIX = /(?:ул\.?|улица|просп\.?|проспект|пр-т|пер\.?|переулок|ш\.?|шоссе|проезд|наб\.?|набережная|бульвар|площадь|пл\.?|микрорайон|мкр\.?)/iu;
...
const NAMED_STREET_WITH_NUMBER = /\b(?:[А-ЯЁ][а-яё-]{3,39}(?:ая|яя|ская|цкая|овская|евская|инская|ская|ская|ский|цкий|овский|евский|инский|ово|ево))\s*,?\s*(?:д\.?\s*)?\d{1,4}[А-Яа-яA-Za-z]?(?:[/\-]\d{1,4})?/u;
...
function extractAddress(text: string): string | null {
    ...
    for (const rawLine of text.split(/\r?\n/).map(x => x.trim()).filter(Boolean)) {
        ...
        // Keep it as an address, but only when the street-type word is explicit.
