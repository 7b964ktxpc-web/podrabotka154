...
function endpoint() { return (process.env.AI_BASE_URL || 'https://api.openai.com/v1').replace(/\/$/, '') + '/chat/completions'; }
function hasAiConfig() { return Boolean(process.env.AI_API_KEY); }
function nullableString(value: unknown) { if (typeof value !== 'string') return null; const v = value.trim(); return v || null; }
function nullableNumber(value: unknown) { if (typeof value !== 'number' || !Number.isFinite(value) || value < 0) return null; return value; }
function validDate(value: unknown) { return typeof value === 'string' && DATE_RE.test(value) && !Number.isNaN(Date.parse(value)) ? value : null; }
function validTime(value: unknown) { return typeof value === 'string' && TIME_RE.test(value) ? value : null; }

function normalize(value: unknown): ParsedJob {
  if (!value || typeof value !== 'object') return EMPTY;
  const x = value as Record<string, unknown>;
  const out: ParsedJob = {
    is_job: Boolean(x.is_job), title: nullableString(x.title), description: nullableString(x.description), category: nullableString(x.category),
    salary_min: nullableNumber(x.salary_min), salary_max: nullableNumber(x.salary_max),
    salary_type: SALARY_TYPES.has(String(x.salary_type)) ? x.salary_type as ParsedJob['salary_type'] : null,
    city: nullableString(x.city), address: nullableString(x.address), date_start: validDate(x.date_start), date_end: validDate(x.date_end),
    time_start: validTime(x.time_start), time_end: validTime(x.time_end),
    ...
    contact_phone: nullableString(x.contact_phone), contact_telegram: nullableString(x.contact_telegram), contact_email: nullableString(x.contact_email),
    confidence: Math.max(0, Math.min(1, Number(x.confidence) || 0)),
  };
  if (out.salary_min !== null && out.salary_max !== null && out.salary_min > out.salary_max) { out.salary_min = null; out.salary_max = null; out.salary_type = null; }
  if (!out.is_job) out.confidence = Math.min(out.confidence, 0.5);
  return out;
}

function normalizedSource(text: string): string { return text.toLocaleLowerCase('ru').replace(/ё/g, 'е'); }
function sourceHasText(text: string, value: string): boolean {
  const source = normalizedSource(text); const candidate = normalizedSource(value).trim(); if (!candidate) return false;
  if (source.includes(candidate)) return true; const tokens = candidate.split(/\s+/).filter(token => token.length >= 3);
...
function sourceHasNumber(text: string, value: number): boolean {
  ...
  const source = text.replace(/(?:\+?7|8)[\d\s().-]{9,}/gu, ' ');
...
function sourceHasContact(text: string, value: string): boolean {
  const source = normalizedSource(text); const candidate = normalizedSource(value).replace(/^@/, ''); if (!candidate) return false;
  if (source.includes(candidate) || source.includes(`@${candidate}`)) return true;
  const sourceDigits = source.replace(/\D/g, ''); const candidateDigits = candidate.replace(/\D/g, ''); return candidateDigits.length >= 7 && sourceDigits.includes(candidateDigits);
}
function sourceHasAddress(text: string, value: string): boolean {
  ...
  const source = normalizeAddress(text); const address = normalizeAddress(value); if (!address) return false; if (source.includes(address)) return true;
  const tokens = address.split(/\s+/).filter(token => token.length >= 3); return tokens.length > 0 && tokens.filter(token => source.includes(token)).length >= Math.min(2, tokens.length);
}
function sourceHasDate(text: string, value: string): boolean {
  const [year, month, day] = value.split('-'); const source = normalizedSource(text);
  const numeric = new RegExp(`(?<!\\d)(?:${day}[-./]${month}|${day}[-./]${month}[-./]${year})(?!\\d)`);
  const monthNames = ['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря']; const named = `${Number(day)} ${monthNames[Number(month) - 1]}`;
...
function sourceHasTime(text: string, value: string): boolean {
  const [hour, minute] = value.split(':'); const source = text.replace(/\u00a0/g, ' '); const h = Number(hour);
  const variants = [`${hour}:${minute}`, `${hour}.${minute}`, `${hour} ${minute}`, `${h}:${minute}`, `${h}.${minute}`, `${h} ${minute}`]; return variants.some(item => source.includes(item));
}

export function groundAiResult(result: ParsedJob, sourceText: string): ParsedJob {
  const out = { ...result }; let removed = 0;
  ...
  if (removed > 0) out.confidence = Math.max(0, out.confidence - Math.min(0.4, removed * 0.08)); return out;
}
function parseJson(text: string) { const cleaned = text.trim().replace(/^```(?:json)?/iu, '').replace(/```$/u, '').trim(); return JSON.parse(cleaned); }
...
export class AIVacancyAgent implements JobParser {
  ...
  async parse(text: string): Promise<ParsedJob> {
    ...
    try {
      const response = await fetch(endpoint(), { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${process.env.AI_API_KEY}` }, body: JSON.stringify({ model: process.env.AI_MODEL || 'gpt-4o-mini', temperature: 0, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: SYSTEM_PROMPT }, { role: 'user', content: JSON.stringify({ text, required_fields: Object.keys(EMPTY) }) }], }), signal: AbortSignal.timeout(30000) });
      if (!response.ok) throw new Error(`AI parser HTTP ${response.status}`); const data = await response.json() as { choices?: Array<{ message?: { content?: string } }> }; const content = data.choices?.[0]?.message?.content; if (!content) throw new Error('AI parser returned empty response');
      return groundAiResult(normalize(parseJson(content)), text);
    } catch { return this.fallback.parse(text); }
  }
}
