...
test('ставка без рубля', () => assert.deepEqual(parseSalary('Ставка 2800'), { salary_min: 2800, salary_max: 2800, salary_type: null }));
test('ставка за смену без рубля', () => assert.deepEqual(parseSalary('Ставка 3300 смена'), { salary_min: 3300, salary_max: 3300, salary_type: 'shift' }));
...
test('другая оплата не объединяется', () => assert.notEqual(fingerprint({ title: 'Грузчик', salary_min: 4000 }), fingerprint({ title: 'Грузчик', salary_min: 5000 })));
