...
create or replace function search_jobs(f jsonb) returns table(job jsonb,total bigint) language sql stable set search_path=public as $$
 with matched as (
 select j.*,
 ...
 from matched m
 order by urgent desc,relevance desc,published_at desc,commercial desc,id
