...
declare j jobs;days int;until_date timestamptz;tz text;begin
 ...
 if p_status='published' then
  ...
  if j.city_id is null then raise exception 'MISSING_CITY';end if;
  if j.address_raw is null and j.salary_min is null and j.salary_max is null
     and j.contact_phone is null and j.contact_telegram is null and j.contact_email is null
     and nullif(coalesce(j.original_text,''),'') is null then raise exception 'EMPTY_JOB';end if;
 end if;
