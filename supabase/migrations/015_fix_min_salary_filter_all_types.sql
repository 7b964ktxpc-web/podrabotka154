-- Apply the minimum salary filter to every salary type.
-- A vacancy may be hourly, shift-based, monthly, or task-based; the filter compares
-- against the explicitly parsed minimum salary value regardless of unit.
...
with matched as (
  ...
  where j.status='published'
    ...
    and (nullif(f->>'min','') is null or (j.salary_min is not null and j.salary_min>=(f->>'min')::numeric))
