...
export function AdminImportButton() {
  ...
  async function runImport() {
    ...
    try {
      ...
      const newCount = results.reduce((sum: number, item: any) => sum + Number(item?.new || 0), 0);
      const failed = results.filter((item: any) => item?.error).length;
      setMessage(failed ? `Импорт завершён: новых ${newCount}, ошибок ${failed}` : `Импорт завершён: новых ${newCount}`);
    } catch (error) {
