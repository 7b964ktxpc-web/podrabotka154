...
const c = serviceDb(), parser = createVacancyParser();
let stopping = false;
process.on('SIGTERM', () => { stopping = true; });
process.on('SIGINT', () => { stopping = true; });
if (process.env.PARSER_PROVIDER && !['conservative', 'ai'].includes(process.env.PARSER_PROVIDER)) throw new Error('Неизвестный parser provider');
...
do { try { await tick(); } catch { console.error('Worker tick failed. Check database configuration.'); if (once) process.exitCode = 1; } if (once || stopping) break; await sleep(Math.max(10, Number(process.env.WORKER_INTERVAL_SECONDS) || 60) * 1000); } while (!stopping);