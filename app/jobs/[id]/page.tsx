...
async function load(id: string) {
    ...
    const c = await db();
    const { data } = check(await c.from('jobs').select('*').eq('id', id).eq('status', 'published').gt('expires_at', new Date().toISOString()).maybeSingle());
    if (!data) notFound();
    return data as Job;
}
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
    ...
    return {
        ...
        openGraph: { title: j.title, description: j.description.slice(0, 155), url: '/jobs/' + id },
    };
}
...
export default async function Detail({ params }: { params: Promise<{ id: string }> }) {
    ...
    return <>
        ...
        <Link href="/jobs" className="muted small">← К объявлениям</Link>
        <div className="detail">
            <article>
                ...
                {j.photo_url && /^https:\/\//.test(j.photo_url) && <Image className="job-image" src={j.photo_url} alt="Фото из объявления" width={760} height={420} unoptimized />}
                <hr className="divider" />
                <h2>Место работы</h2>
                <Address job={j} />
            ...
            <aside className="aside">
                ...
                <p className="small muted">Уточните, актуальна ли вакансия, и договоритесь об условиях.</p>
                {phone && <Contact id={id} type="phone_click" href={'tel:' + phone.replace(/[^+\d]/g, '')}>Позвонить</Contact>}
                {tg && <Contact id={id} type="telegram_click" href={'https://t.me/' + tg}>Написать в Telegram</Contact>}
                {email && <Contact id={id} type="email_click" href={'mailto:' + email}>Написать на email</Contact>}
                {!phone && !tg && !email && <p>Прямые контакты не указаны. Проверьте оригинал объявления.</p>}
                <hr className="divider" />
                <div className="row"><Favorite id={id} initial={Boolean(f)} /><span>Сохранить</span></div>
                <p><Share id={id} title={j.title} /></p>
                <p className="small muted">Не переводите деньги за доступ к работе.</p>
            </aside>
        </div>
    </>;
}
