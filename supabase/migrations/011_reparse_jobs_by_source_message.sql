...
begin
  ...
  if j is not null then
    update jobs set
      ...
      city_id=s.city_id,
      city=c,
      address_raw=p_result->>'address',
      address_normalized=p_result->>'address',
      salary_min=(p_result->>'salary_min')::numeric,
      salary_max=(p_result->>'salary_max')::numeric,
      salary_type=p_result->>'salary_type',
      employment_type=p_result->>'employment_type',
      payment_type=p_result->>'payment_type',
      date_start=nullif(p_result->>'date_start','')::date,
      date_end=nullif(p_result->>'date_end','')::date,
      time_start=nullif(p_result->>'time_start','')::time,
      time_end=nullif(p_result->>'time_end','')::time,
      contact_phone=p_result->>'contact_phone',
      contact_telegram=p_result->>'contact_telegram',
      contact_email=p_result->>'contact_email',
      source_url=m.message_url,
      original_text=m.message_text,
