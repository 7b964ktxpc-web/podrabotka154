alter table public.jobs add column if not exists map_url text;
...
begin
  ...
  update jobs set
    ...
    category=nullif(p->>'category','')::uuid,
    salary_min=nullif(p->>'salary_min','')::numeric, salary_max=nullif(p->>'salary_max','')::numeric,
    salary_type=nullif(p->>'salary_type',''), address_raw=nullif(trim(p->>'address_raw'),''),
    address_normalized=nullif(trim(p->>'address_raw'),''), map_url=nullif(trim(p->>'map_url'),''),
    employment_type=nullif(p->>'employment_type',''), payment_type=nullif(p->>'payment_type',''),
    date_start=nullif(p->>'date_start','')::date, date_end=nullif(p->>'date_end','')::date,
    time_start=nullif(p->>'time_start','')::time, time_end=nullif(p->>'time_end','')::time,
    contact_phone=nullif(p->>'contact_phone',''), contact_telegram=nullif(p->>'contact_telegram',''),
    contact_email=nullif(p->>'contact_email',''), photo_url=nullif(p->>'photo_url',''),
